...
var editor_plugin: EditorPlugin:
	set(val): set_editor_plugin(val)
var options:
	set(val): _set_options(val)
...
func get_handle_name(_gizmo: EditorNode3DGizmo, index: int) -> String:
...
func commit_handle(gizmo: EditorNode3DGizmo, index: int, restore, _cancel: bool = false) -> void:
	var path = gizmo.get_spatial_node()
	if not path:
		return
