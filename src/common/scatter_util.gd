...
static func ensure_output_root_exists(s: Node) -> void:
	...
	if s.output_root and is_instance_valid(s.output_root) and s.has_node(NodePath(s.output_root.name)):
	...
	if s.output_root:
		if s.has_node(NodePath(s.output_root.name)):
			s.remove_child(s.output_root.name)
...
static func get_or_create_item_root(item) -> Node3D:
...
static func get_or_create_multimesh(item, count: int) -> MultiMeshInstance3D:
